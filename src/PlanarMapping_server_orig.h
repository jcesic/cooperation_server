...
public:
	...
	void load_models(){
		...
		br_pog_old=br_pog;
		for (int i = br_pog; i < models_in_trajectory; i++){
			int brloc = i;
			MODEL tmp_model;
			num_deleted = 0;
			...
			cout << "load model: " << temp_path+buffer << endl;
...
/*				local_models[i].changed=true;
				...
				for(int j = 0; j < local_models[i].local_planes.size();j++){
					...
					if((tmp_glob_id!=-1) && !(global_planes[tmp_glob_id].reformat) && global_planes[tmp_glob_id].active){
						printf("reformating GLOBAL PLANE %d\n",tmp_glob_id);
