...
class slam_callback{
	string f_odom, f_odom_t, f_odom_cov, f_img,odom_location,true_location,img_location,seq_cov_location;
	boost::format odom_format_, ground_format_, pcl_time_, odom_time_, img_format_, cov_format_;
	ros::NodeHandle n_;
...
public:
	slam_callback(){
		...
		if(!ros::param::get("/seq_cov_location", seq_cov_location)){
			seq_cov_location  = "/media/kruno/Data/TEST/SLAM/SeqCov";
		}
	...
	void send_data(){
		...
		for(int i = 0; i < br_img; i++){
			...
			if (callback_client.call(servCallback)){
				...
				if(servCallback.response.odom_state_id){
					if(num_loops<max_loops){
						printf("New loop\n");
						i = servCallback.response.odom_state_id;
						num_loops++;
					}
				}
			}else{
