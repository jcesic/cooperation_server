...
public:
	slam_callback(){
		...
		callback_client = n_.serviceClient<cooperation_server::SlamCallback>("slam_callback_1");
		odom_pub = n_.advertise<nav_msgs::Odometry>("/slam/odom_1",1);
		image_pub = n_.advertise<sensor_msgs::Image>("/slam/image_1",1);

		if(!ros::param::get("/odom_location_2", odom_location)){
			odom_location  = "/media/kruno/Data/TEST/SLAM/Odom";
		}
		if(!ros::param::get("/pcd_location_2", pcd_location)){
			pcd_location  = "/media/kruno/Data/TEST/SLAM/PCD";
		}
		if(!ros::param::get("/img_location_2", img_location)){
	...
	void send_data(){
		...
		for(int i = br_img; (i < 3375 && ros::ok()); i++){
...
int main(int argc, char* argv[]){
	...
	ros::init(argc, argv, "slam_callback_agent2");
