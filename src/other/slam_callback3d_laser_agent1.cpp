...
public:
	slam_callback(){
		...
		callback_client = n_.serviceClient<cooperation_server::SlamCallback>("slam_callback_0");
		odom_pub = n_.advertise<nav_msgs::Odometry>("/slam/odom_0",1);
		//true_pub = n_.advertise<nav_msgs::Odometry>("/true/odom",1);
		image_pub = n_.advertise<sensor_msgs::Image>("/slam/image_0",1);

		if(!ros::param::get("/agent_id", agent_id)){
			agent_id  = 0;
		}

		if(!ros::param::get("/odom_location_1", odom_location)){
			odom_location  = "/media/kruno/Data/TEST/SLAM/Odom";
		}
		if(!ros::param::get("/pcd_location_1", pcd_location)){
			pcd_location  = "/media/kruno/Data/TEST/SLAM/PCD";
		}
		if(!ros::param::get("/img_location_1", img_location)){
		...
		cout << "Num:                " << br_img << endl;
		cout << "Odom loaded from:   " << odom_location << endl;
		cout << "Pcd loaded from:    " << pcd_location << endl;
...
int main(int argc, char* argv[]){
	...
	ros::init(argc, argv, "slam_callback_agent1");
