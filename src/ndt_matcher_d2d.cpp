...
bool NDTMatcherD2D::storeNDT(pcl::PointCloud<pcl::PointXYZ>& target){
        ...
        bool useInitialGuess)
{
    ...
    struct timeval tv_start0, tv_end0, tstart_ndt,tend_ndt;
    double time_load =0, time_match=0, time_combined=0, time_ndt = 0;
...
#endif
    {
        ...
        for(int r_ctr = resolutions.size()-1; r_ctr >=0;  r_ctr--)
        {
            ...
            DELTA_SCORE = 10e-3*current_resolution;
            lfd1 = current_resolution*2.5;
            lfd2 = 0.1/current_resolution;
            ...
            gettimeofday(&tstart_ndt,NULL);
            targetNDT.loadPointCloud( target );
            targetNDT.computeNDTCells();
            gettimeofday(&tend_ndt,NULL);
            time_ndt += (tend_ndt.tv_sec-tstart_ndt.tv_sec)+(tend_ndt.tv_usec-tstart_ndt.tv_usec)/1000000.;
...
/*
    ...
    cov = cov.inverse();//*fabsf(scoreNDT(sourceNDTN,targetNDT)*2/3);
    FILE *pFile;
    float score = fabsf(scoreNDT(sourceNDTN,targetNDT)*2/3);
    pFile = fopen("/media/kruno/Data/TEST/FER/NDT/score.txt","a");
    fprintf(pFile,"%f\n",score);
    fclose(pFile);
